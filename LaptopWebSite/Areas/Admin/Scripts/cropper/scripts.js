$(function () {
    var cropper = function () {
        //Чи открито діалогово вікно для кропу
        var isCropped = false;
        var loader = {
            //Для початку анімації загрузки фото
            show: function () {
                $("#loading").show();
            },
            //Для закривання анімації загрузки фото
            hide: function () {
                $("#loading").hide();
            }
        };
        ...
        var dialog = {
            //Шоб воно було як на головному екрані
            body: document.querySelector('body'),
            //Для відкриття самого кропу
            show: function () {
                this.body.classList.toggle("open");
                $(".containerCrop").show();
            },
            //Для закривання самого кропу
            hide: function () {
        ...
        var server = {
            sendImage: function (base64) {
                ...
                $.ajax({
                    url: "/Admin/AdminPanel/UploadBase64",
                    type: "POST",
                    data: {
                    ...
                    success: function (result) {
                        ...
                        itemImage += '<div class="col-md-2 uploadImage"  data-id="' + result.image + '">' +
                            '<div class="thumbnail">' +
                            '<i class="fa fa-times  icon-delete" aria-hidden="true" style="color:gray; font-size:20px; margin:5px; margin-right:0px"></i>' +
            ...
            deleteImage: function (id) {
                ...
                //var image = $(imageCurrent).find(".uploadimage")[0];
                $.ajax({
                    url: "/Admin/AdminPanel/DeleteImageAjax",
                    type: "POST",
                    data: {
                        __RequestVerificationToken: token,
                        id: id
